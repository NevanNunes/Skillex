          ...
          <>
            <GlassCard variant="strong" className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
              ...
              <div className="flex-1">
                <h1 className="font-display text-2xl font-bold">{user.full_name ?? user.username}</h1>
                <p className="text-muted-foreground">@{user.username} · {user.university}</p>
                <p className="mt-2 max-w-prose">{user.bio}</p>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-center">
                  <div className="font-display text-2xl font-bold">{user.reputation?.toFixed(1)}</div>
                  <RatingStars value={user.reputation ?? 0} />
                </div>
            ...
            <section>
              <h2 className="font-display text-lg font-semibold mb-3">Teaches</h2>
              <div className="flex flex-wrap gap-2">
                {(teach ?? []).map((t) => (
                  <Badge key={t.id} variant="secondary" className="text-sm py-1.5 px-3">{t.skill.name} · {t.level}</Badge>
                ))}
                {(teach ?? []).length === 0 && <p className="text-sm text-muted-foreground">No public skills yet.</p>}
              </div>
            </section>
            ...
            <section>
              ...
              {reviews?.results.length ? (
                  ...
                  {reviews.results.map((r) => (
                    <GlassCard key={r.id} className="space-y-2">
                      <div className="flex items-center gap-2">
                        <UserAvatar user={r.reviewer} size="sm" />
                        <div className="flex-1">
                          <p className="text-sm font-semibold">{r.reviewer.full_name ?? r.reviewer.username}</p>
                ...
                <EmptyState title="No reviews yet" description="Reviews will appear here once sessions are completed." />
              )}
            </section>
          </>
        )}
