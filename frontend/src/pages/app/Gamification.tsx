...
export default function Gamification() {
  ...
  const pct = me.data.xp / me.data.next_level_xp;
  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard variant="strong" className="flex flex-col sm:flex-row items-center gap-6">
        <ScoreRing value={pct} size={120} label={`L${me.data.level}`} />
        <div className="flex-1 space-y-1">
          <p className="font-display text-2xl font-bold">{me.data.xp} XP</p>
          <p className="text-sm text-muted-foreground">Next level at {me.data.next_level_xp} XP · {me.data.next_level_xp - me.data.xp} to go</p>
          <p className="text-sm">Reputation <span className="font-semibold">{me.data.reputation.toFixed(2)} ★</span></p>
        </div>
      </GlassCard>
      ...
      <section>
        ...
        <div className="grid sm:grid-cols-3 lg:grid-cols-4 gap-3 stagger">
          {(badges.data ?? []).map((b) => (
            <GlassCard key={b.id} className="text-center space-y-2">
        ...
        <GlassCard className="divide-y divide-border/50">
          {(history.data?.results ?? []).map((e) => (
            <div key={e.id} className="flex items-center justify-between py-2.5 first:pt-0 last:pb-0">
              ...
              <span className="font-display font-bold text-primary">+{e.delta}</span>
            </div>
          ))}
        </GlassCard>
      </section>
